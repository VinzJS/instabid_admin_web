 ...
 * of users stored in the users collection.
 *
 * GET: Retrieve a list of records from the users collection.
 *      The request body should contain the following:
 *      {
 *          "limit": 10, // The number of records to retrieve
 *          "skip": 0 // The number of records to skip
 *      }
 *
 * POST: Create a new record in the users collection.
 *       The request body should contain the following:
 *       {
 *           "name": "John Doe",
 ...
 *       }
 *
 * PATCH: Update an existing record in the users collection.
 *        The request body should contain the following:
 *        {
 ...
 *        }
 *
 * DELETE: Delete a record in the users collection. The request
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($_GET['recordId'])) {
        ...
        if ($recordId == 'index.php') {
            ...
            try {
                $countQuery = "SELECT COUNT(*) as totalItems FROM users";
                $countResult = $conn->query($countQuery);
                $totalItems = $countResult->fetch_assoc()['totalItems'];
                $totalPages = ceil($totalItems / $perPage);

                $query = "SELECT id, email, name, username, isStoreOwner, created, updated FROM users LIMIT ? OFFSET ?";
        ...
        try {
            $query = "SELECT id, email, name, username, isStoreOwner, created, updated FROM users WHERE id = ? LIMIT 1";
            $stmt = $conn->prepare($query);
...
if ($_SERVER['REQUEST_METHOD'] == 'PATCH') {
    if (isset($_GET['recordId'])) {
        ...
        try {
            $query = "UPDATE users SET name = ?, username = ?, isStoreOwner = ? WHERE id = ?";
            $stmt = $conn->prepare($query);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $insertQuery = "INSERT INTO users (id, email, name, password) VALUES (?, ?, ?, ?)";
